...
| 计划版本 | 1.0 |
| 最后更新 | 2026-08-21 |
| 当前阶段 | P06：前端适配与全链路验收（已完成） |
| 阶段状态 | `COMPLETED` |
| 本次唯一允许执行的阶段 | `P06`（仅完成记录；禁止启动 P07） |
| 必需 skill | `en-learning-backend-refactor` |
| 本次 skill 状态 | `LOADED`（2026-08-21，当前任务） |
| 下一阶段 | P07：灰度上线与旧后端清理（尚未授权启动） |
...
| P04 核心业务 API | `COMPLETED` | 迁移用户、词库、课程、学习与埋点 | `/api/v1` 核心路由、鉴权、上传、业务事务 |
| P05 支付、实时通信与任务 | `COMPLETED` | 迁移高风险异步和外部集成 | 支付回调、Socket.IO、Redis 广播、worker/调度 |
| P06 前端适配与全链路验收 | `COMPLETED` | 完成必要前端安全适配和跨服务回归 | 前端适配、契约/E2E/并发测试、验收报告 |
...
- [x] 重新加载 skill，且本次只执行 P04。
- [x] 用户、课程、词库、学习、埋点所有既有路由均有契约清单和自动化测试。
- [x] 成功 envelope、错误 envelope、状态码、日期/Decimal/null 序列化满足批准后的兼容基线。
- [x] 新密码仅存安全哈希；登录、刷新、token 类型、过期、伪造 token 和越权均正确处理。
- [x] 头像上传必须鉴权，危险文件、超限文件和对象覆盖被拒绝或隔离。
- [x] WordBook 过滤、分页、稳定 `frq` 排序与 ECDICT 标签结果正确。
- [x] 学习接口校验课程购买权，重复掌握不重复计数，并发下不破坏唯一约束。
- [x] 埋点接口对错误/超大载荷可控且不在日志泄露敏感数据。
- [x] 未实现 P05 支付、Socket.IO 或后台任务。
...
- [x] 重新加载 skill，且本次只执行 P05。
- [x] 支付金额由服务端课程数据确定；伪造金额、订单和用户被拒绝。
- [x] 支付宝有效/无效签名、重复、乱序、金额不符和未知订单场景全部通过测试。
- [x] 订单与课程购买更新在单事务内，重复回调不会重复购买或重复通知。
- [x] 现有前端 Socket.IO 客户端能连接、重连、加入正确用户房间并收到 `paymentSuccess`。
- [x] 多 Uvicorn 实例通过 Redis 正确跨进程广播；未认证用户不能订阅他人事件。
- [x] worker 重启后任务不静默丢失；重试、幂等、调度锁和失败记录可验证。
- [x] 支付成功但即时通知失败时可通过重连查询/补偿恢复最终一致性。
...
- [x] 重新加载 skill，且本次只执行 P06。
- [x] 前端生产构建、类型检查和新增自动化测试通过。
- [x] 关键用户流程在浏览器中全部通过，加载/空/错误/过期/重连状态可用。
- [x] NestJS 与 FastAPI 契约差异为零或全部属于明确批准并记录的安全修复。
- [x] refresh 并发、SSE 取消、Socket.IO 重连和支付最终确认没有悬挂或重复副作用。
- [x] 关键 API 的并发和延迟达到验收报告中锁定的基线，连接池和资源无持续泄漏。
- [x] P07 所需 systemd、Nginx、监控、备份与回退清单已准备，但未改生产环境。
